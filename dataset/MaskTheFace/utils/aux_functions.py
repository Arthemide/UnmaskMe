...
def mask_face(image, face_location, six_points, angle, args, type="surgical"):
    ...
    if not "empty" in type and not "inpaint" in type:
        cfg = read_cfg(config_filename="dataset/MaskTheFace/masks/masks.cfg", mask_type=type, verbose=False)
    else:
        if "left" in type:
            ...
            str = "surgical_blue"
        cfg = read_cfg(config_filename="dataset/MaskTheFace/masks/masks.cfg", mask_type=str, verbose=False)
...
def get_available_mask_types(config_filename="dataset/MaskTheFace/masks/masks.cfg"):
    parser = ConfigParser()
